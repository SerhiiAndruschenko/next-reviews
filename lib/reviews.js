...
export async function getSlugs() {
    const response = await fetch("https://bookwarm-cms.testapp.fun/wp-json/wp/v2/posts");
...
export async function getReview(slug) {
    // Fetch post data
    const response = await fetch(`https://bookwarm-cms.testapp.fun/wp-json/wp/v2/posts?slug=${slug}`);
    const data = await response.json();
